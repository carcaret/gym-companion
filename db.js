const defaultDBData = {
  "auth": {
    "username": "carlos",
    "passwordHash": "<SHA256_DE_SAL+CONTRASEÃ‘A>"
  },
  "exercises": {
    "abductores": {
      "id": "abductores",
      "name": "Abductores"
    },
    "adductores": {
      "id": "adductores",
      "name": "Adductores"
    },
    "apertura_con_cable": {
      "id": "apertura_con_cable",
      "name": "Apertura con Cable"
    },
    "apertura_maquina": {
      "id": "apertura_maquina",
      "name": "Apertura en MÃ¡quina"
    },
    "aperturas_mancuernas": {
      "id": "aperturas_mancuernas",
      "name": "Aperturas con Mancuernas"
    },
    "cable_row": {
      "id": "cable_row",
      "name": "Cable Row"
    },
    "crunches": {
      "id": "crunches",
      "name": "Crunches"
    },
    "curl_de_biceps_con_barra": {
      "id": "curl_de_biceps_con_barra",
      "name": "Curl de BÃ­ceps con Barra"
    },
    "curl_de_biceps_mancuerna": {
      "id": "curl_de_biceps_mancuerna",
      "name": "Curl de BÃ­ceps con Mancuerna"
    },
    "curl_femoral_sentado": {
      "id": "curl_femoral_sentado",
      "name": "Curl Femoral Sentado"
    },
    "elevacion_rodillas": {
      "id": "elevacion_rodillas",
      "name": "ElevaciÃ³n de Rodillas"
    },
    "elevaciones_gemelos": {
      "id": "elevaciones_gemelos",
      "name": "Elevaciones de Gemelos"
    },
    "elevaciones_laterales_con_cable": {
      "id": "elevaciones_laterales_con_cable",
      "name": "Elevaciones Laterales con Cable"
    },
    "elevaciones_laterales_con_mancuernas": {
      "id": "elevaciones_laterales_con_mancuernas",
      "name": "Elevaciones Laterales con Mancuernas"
    },
    "encogimiento_trapecio_mancuernas": {
      "id": "encogimiento_trapecio_mancuernas",
      "name": "Encogimiento de Trapecios con Mancuernas"
    },
    "extension_de_cuadriceps": {
      "id": "extension_de_cuadriceps",
      "name": "ExtensiÃ³n de CuÃ¡driceps"
    },
    "extensiones_de_espalda": {
      "id": "extensiones_de_espalda",
      "name": "Extensiones de Espalda"
    },
    "face_pull": {
      "id": "face_pull",
      "name": "Face Pull"
    },
    "hack_squat": {
      "id": "hack_squat",
      "name": "Hack Squat"
    },
    "hammer_row_neutral": {
      "id": "hammer_row_neutral",
      "name": "Hammer Row Neutral"
    },
    "hammer_row_prono": {
      "id": "hammer_row_prono",
      "name": "Hammer Row Prono"
    },
    "hip_thrust": {
      "id": "hip_thrust",
      "name": "Hip Thrust"
    },
    "jalon_al_pecho": {
      "id": "jalon_al_pecho",
      "name": "JalÃ³n al Pecho"
    },
    "landmine_press": {
      "id": "landmine_press",
      "name": "Landmine Press"
    },
    "lat_prayers": {
      "id": "lat_prayers",
      "name": "Lat Prayers"
    },
    "overhead_extension_triceps": {
      "id": "overhead_extension_triceps",
      "name": "Overhead Extension TrÃ­ceps"
    },
    "peso_muerto_barra": {
      "id": "peso_muerto_barra",
      "name": "Peso Muerto con Barra"
    },
    "peso_muerto_rumano": {
      "id": "peso_muerto_rumano",
      "name": "Peso Muerto Rumano"
    },
    "plancha": {
      "id": "plancha",
      "name": "Plancha"
    },
    "prensa_de_piernas": {
      "id": "prensa_de_piernas",
      "name": "Prensa de Piernas"
    },
    "press_banca_barra": {
      "id": "press_banca_barra",
      "name": "Press de Banca con Barra"
    },
    "press_banca_mancuernas": {
      "id": "press_banca_mancuernas",
      "name": "Press de Banca con Mancuernas"
    },
    "press_de_hombros_mancuernas_sentado": {
      "id": "press_de_hombros_mancuernas_sentado",
      "name": "Press de Hombros con Mancuernas Sentado"
    },
    "press_de_hombros_maquina": {
      "id": "press_de_hombros_maquina",
      "name": "Press de Hombros en MÃ¡quina"
    },
    "press_inclinado_mancuernas": {
      "id": "press_inclinado_mancuernas",
      "name": "Press Inclinado con Mancuernas"
    },
    "press_inclinado_maquina": {
      "id": "press_inclinado_maquina",
      "name": "Press Inclinado en MÃ¡quina"
    },
    "press_inclinado_palanca": {
      "id": "press_inclinado_palanca",
      "name": "Press Inclinado en Palanca"
    },
    "press_militar_barra": {
      "id": "press_militar_barra",
      "name": "Press Militar con Barra"
    },
    "pullover_polea": {
      "id": "pullover_polea",
      "name": "Pullover en Polea"
    },
    "remo_con_barra": {
      "id": "remo_con_barra",
      "name": "Remo con Barra"
    },
    "remo_con_mancuerna": {
      "id": "remo_con_mancuerna",
      "name": "Remo con Mancuerna"
    },
    "remo_con_mancuerna_con_apoyo_en_el_pecho": {
      "id": "remo_con_mancuerna_con_apoyo_en_el_pecho",
      "name": "Remo con Mancuerna con Apoyo en el Pecho"
    },
    "remo_con_polea": {
      "id": "remo_con_polea",
      "name": "Remo con Polea"
    },
    "remo_en_maquina": {
      "id": "remo_en_maquina",
      "name": "Remo en MÃ¡quina"
    },
    "reverse_delt_fly": {
      "id": "reverse_delt_fly",
      "name": "Reverse Delt Fly"
    },
    "reverse_delt_maquina": {
      "id": "reverse_delt_maquina",
      "name": "Reverse Delt en MÃ¡quina"
    },
    "russian_twists": {
      "id": "russian_twists",
      "name": "Russian Twists"
    },
    "sentadilla_barra": {
      "id": "sentadilla_barra",
      "name": "Sentadilla con Barra"
    },
    "sentadilla_bulgara": {
      "id": "sentadilla_bulgara",
      "name": "Sentadilla BÃºlgara"
    },
    "sentadilla_frontal": {
      "id": "sentadilla_frontal",
      "name": "Sentadilla Frontal"
    },
    "skullcrusher_con_barra": {
      "id": "skullcrusher_con_barra",
      "name": "Skullcrusher con Barra"
    },
    "t_bar_row": {
      "id": "t_bar_row",
      "name": "T-Bar Row"
    },
    "triceps_polea": {
      "id": "triceps_polea",
      "name": "TrÃ­ceps en Polea"
    },
    "zancadas_con_mancuernas": {
      "id": "zancadas_con_mancuernas",
      "name": "Zancadas con Mancuernas"
    },
    "encogimiento_trapecio_barra": {
      "id": "encogimiento_trapecio_barra",
      "name": "Encogimiento de Trapecios con Barra"
    }
  },
  "routines": {
    "LUNES": [
      "prensa_de_piernas",
      "press_banca_mancuernas",
      "hammer_row_neutral",
      "landmine_press",
      "curl_femoral_sentado"
    ],
    "MIERCOLES": [
      "abductores",
      "adductores",
      "press_inclinado_mancuernas",
      "jalon_al_pecho",
      "reverse_delt_fly",
      "extensiones_de_espalda",
      "elevaciones_laterales_con_cable"
    ],
    "VIERNES": [
      "extension_de_cuadriceps",
      "press_inclinado_maquina",
      "pullover_polea",
      "press_de_hombros_mancuernas_sentado",
      "hip_thrust",
      "encogimiento_trapecio_barra"
    ]
  },
  "history": [
    {
      "date": "2024-09-23",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "remo_con_barra",
          "name": "Remo con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "elevaciones_laterales_con_mancuernas",
          "name": "Elevaciones Laterales con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "plancha",
          "name": "Plancha",
          "series": 3,
          "reps": {
            "expected": 30,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2024-09-25",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2024-09-27",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2024-10-14",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 42.5
        },
        {
          "exercise_id": "remo_con_barra",
          "name": "Remo con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "plancha",
          "name": "Plancha",
          "series": 3,
          "reps": {
            "expected": 30,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2024-10-16",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 21.5
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 45
        }
      ],
      "completed": true
    },
    {
      "date": "2024-10-18",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 2.5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 36
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        }
      ],
      "completed": true
    },
    {
      "date": "2024-10-21",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 42.5
        },
        {
          "exercise_id": "remo_con_barra",
          "name": "Remo con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "plancha",
          "name": "Plancha",
          "series": 3,
          "reps": {
            "expected": 30,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2024-10-23",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 21.5
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 50
        }
      ],
      "completed": true
    },
    {
      "date": "2024-10-25",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 2.5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 36
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        }
      ],
      "completed": true
    },
    {
      "date": "2024-10-28",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "remo_con_barra",
          "name": "Remo con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "plancha",
          "name": "Plancha",
          "series": 3,
          "reps": {
            "expected": 30,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2024-10-30",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 21.5
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 50
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-01",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-04",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "remo_con_barra",
          "name": "Remo con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "plancha",
          "name": "Plancha",
          "series": 3,
          "reps": {
            "expected": 30,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-06",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 21.5
        },
        {
          "exercise_id": "lat_prayers",
          "name": "Lat Prayers",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 50
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-08",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-11",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "remo_con_barra",
          "name": "Remo con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "plancha",
          "name": "Plancha",
          "series": 3,
          "reps": {
            "expected": 30,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-13",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 21.5
        },
        {
          "exercise_id": "lat_prayers",
          "name": "Lat Prayers",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 50
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-15",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-18",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "remo_con_barra",
          "name": "Remo con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "plancha",
          "name": "Plancha",
          "series": 3,
          "reps": {
            "expected": 30,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-20",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 21.5
        },
        {
          "exercise_id": "lat_prayers",
          "name": "Lat Prayers",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 54
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-22",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-25",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "remo_con_barra",
          "name": "Remo con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "plancha",
          "name": "Plancha",
          "series": 3,
          "reps": {
            "expected": 30,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-27",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 22.5
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 54
        }
      ],
      "completed": true
    },
    {
      "date": "2024-11-29",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 7,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        }
      ],
      "completed": true
    },
    {
      "date": "2024-12-02",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "plancha",
          "name": "Plancha",
          "series": 3,
          "reps": {
            "expected": 30,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2024-12-04",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 54
        }
      ],
      "completed": true
    },
    {
      "date": "2024-12-06",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        }
      ],
      "completed": true
    },
    {
      "date": "2024-12-09",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "plancha",
          "name": "Plancha",
          "series": 3,
          "reps": {
            "expected": 30,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2024-12-11",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 80
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 54
        }
      ],
      "completed": true
    },
    {
      "date": "2024-12-13",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        }
      ],
      "completed": true
    },
    {
      "date": "2024-12-16",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 43
        }
      ],
      "completed": true
    },
    {
      "date": "2024-12-18",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 80
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 54
        }
      ],
      "completed": true
    },
    {
      "date": "2024-12-20",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        }
      ],
      "completed": true
    },
    {
      "date": "2024-12-30",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 43
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-01",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 80
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 54
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-03",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-06",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 43
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-08",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 80
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 54
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-10",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 7.5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "russian_twists",
          "name": "Russian Twists",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-13",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 43
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-15",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 80
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 54
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-17",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 7.5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-20",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 43
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-22",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 85
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 54
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-24",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 7.5
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-27",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-29",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 85
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 22.5
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 54
        }
      ],
      "completed": true
    },
    {
      "date": "2025-01-31",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-02-03",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        }
      ],
      "completed": true
    },
    {
      "date": "2025-02-05",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 59
        }
      ],
      "completed": true
    },
    {
      "date": "2025-02-07",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-02-10",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 64
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 50
        }
      ],
      "completed": true
    },
    {
      "date": "2025-02-12",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 68
        }
      ],
      "completed": true
    },
    {
      "date": "2025-02-14",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-02-17",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 64
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-02-19",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 68
        }
      ],
      "completed": true
    },
    {
      "date": "2025-02-21",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-03",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-05",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 68
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-07",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-10",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-12",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 68
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-14",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-17",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-19",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 68
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-21",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-24",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-26",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 68
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-28",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-03-31",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-02",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 68
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-04",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-07",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-09",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 68
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-11",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-14",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "sentadilla_barra",
          "name": "Sentadilla con Barra",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "press_banca_barra",
          "name": "Press de Banca con Barra",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "remo_con_polea",
          "name": "Remo con Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "peso_muerto_rumano",
          "name": "Peso Muerto Rumano",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 60
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-16",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "peso_muerto_barra",
          "name": "Peso Muerto con Barra",
          "series": 4,
          "reps": {
            "expected": 5,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_palanca",
          "name": "Press Inclinado en Palanca",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 68
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-18",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "sentadilla_frontal",
          "name": "Sentadilla Frontal",
          "series": 4,
          "reps": {
            "expected": 6,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "skullcrusher_con_barra",
          "name": "Skullcrusher con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 35
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 41
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-21",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_con_mancuerna_con_apoyo_en_el_pecho",
          "name": "Remo con Mancuerna con Apoyo en el Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-23",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 73
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-25",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 19,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-28",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 120
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_con_mancuerna_con_apoyo_en_el_pecho",
          "name": "Remo con Mancuerna con Apoyo en el Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-04-30",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 73
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-02",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 19,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-05",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 120
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 19,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-07",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 73
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-09",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 11.25
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-12",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_con_mancuerna_con_apoyo_en_el_pecho",
          "name": "Remo con Mancuerna con Apoyo en el Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-14",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 73
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-16",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 11.3
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 19,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-19",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_con_mancuerna_con_apoyo_en_el_pecho",
          "name": "Remo con Mancuerna con Apoyo en el Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-21",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 73
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-23",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 11.3
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 19,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-26",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_con_mancuerna_con_apoyo_en_el_pecho",
          "name": "Remo con Mancuerna con Apoyo en el Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-28",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 73
        }
      ],
      "completed": true
    },
    {
      "date": "2025-05-30",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 11.3
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 19,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-02",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_con_mancuerna_con_apoyo_en_el_pecho",
          "name": "Remo con Mancuerna con Apoyo en el Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-04",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 73
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-06",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 11.3
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 19,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-09",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_con_mancuerna_con_apoyo_en_el_pecho",
          "name": "Remo con Mancuerna con Apoyo en el Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-11",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 73
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-13",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 11.3
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 19,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-16",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_con_mancuerna_con_apoyo_en_el_pecho",
          "name": "Remo con Mancuerna con Apoyo en el Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-18",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_de_hombros_maquina",
          "name": "Press de Hombros en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12.5
        },
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 73
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-20",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "aperturas_mancuernas",
          "name": "Aperturas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "remo_con_mancuerna",
          "name": "Remo con Mancuerna",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 11.3
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 19,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-23",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 130
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              3
            ]
          },
          "weight": 22.5
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 17,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-25",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              9
            ]
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "curl_de_biceps_con_barra",
          "name": "Curl de BÃ­ceps con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-27",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 77
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 13.6
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 63
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 6.8
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-06-30",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              10,
              10,
              10
            ]
          },
          "weight": 130
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": [
              3
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-02",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              9
            ]
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 45
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-04",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 77
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 13.6
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 47.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 6.8
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-07",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 130
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              7
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": [
              3
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-09",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              10
            ]
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              2008
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-11",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              10
            ]
          },
          "weight": 77
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 13.6
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 47.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 6.8
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-14",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 130
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              7
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": [
              3
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-16",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              10
            ]
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": [
              11,
              11,
              2008
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-18",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              10
            ]
          },
          "weight": 77
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 13.6
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              12
            ]
          },
          "weight": 70
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 47.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 6.8
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-21",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 130
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              7
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": [
              3
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-23",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              11,
              11,
              11,
              10
            ]
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": [
              11,
              11,
              2008
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-25",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              10
            ]
          },
          "weight": 77
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 13.6
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              12
            ]
          },
          "weight": 70
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 47.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 6.8
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-28",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 130
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              7
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": [
              3
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-07-30",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": [
              11,
              11,
              2008
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-01",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              10
            ]
          },
          "weight": 77
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 13.6
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 47.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 6.8
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-04",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 130
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              7
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": [
              3
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-06",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              11,
              11,
              2008
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-08",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 77
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 13.6
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 47.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 6.8
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-11",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 130
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              8
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": [
              3
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-13",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              8
            ]
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              11,
              11,
              2008
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-15",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 77
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 13.6
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 70
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 47.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 6.8
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-18",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 130
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              8
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": [
              3
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "elevacion_rodillas",
          "name": "ElevaciÃ³n de Rodillas",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-20",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "zancadas_con_mancuernas",
          "name": "Zancadas con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              8
            ]
          },
          "weight": 20
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              14,
              13,
              13
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-22",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 77
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 13.6
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 70
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 47.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 6.8
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-25",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 130
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              9,
              8
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-27",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "sentadilla_bulgara",
          "name": "Sentadilla BÃºlgara",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              14,
              14,
              10
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-08-29",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 47.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 6.8
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-01",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              10
            ]
          },
          "weight": 130
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10,
              9
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-03",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "sentadilla_bulgara",
          "name": "Sentadilla BÃºlgara",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10,
              9
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              14,
              14,
              10
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-05",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 47.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 6.8
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-08",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 130
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-10",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "sentadilla_bulgara",
          "name": "Sentadilla BÃºlgara",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              14,
              14,
              13
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-12",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              10
            ]
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10,
              8
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 77
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 47.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              2008
            ]
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-15",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 140
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-17",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "sentadilla_bulgara",
          "name": "Sentadilla BÃºlgara",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              14,
              14,
              13
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-19",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 47.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-22",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10,
              8
            ]
          },
          "weight": 140
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              8
            ]
          },
          "weight": 24
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 20
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-24",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "sentadilla_bulgara",
          "name": "Sentadilla BÃºlgara",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              10
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              14,
              14,
              13
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-26",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11,
              11
            ]
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": [
              13,
              13,
              11
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              2009
            ]
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-09-29",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 140
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22.5
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-10-01",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "sentadilla_bulgara",
          "name": "Sentadilla BÃºlgara",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              14,
              14,
              13
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-10-03",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11,
              11
            ]
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": [
              13,
              13,
              11
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              2009
            ]
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-10-06",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 140
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 22.5
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              11,
              11
            ]
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              14,
              14,
              13
            ]
          },
          "weight": 17.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-10-08",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "sentadilla_bulgara",
          "name": "Sentadilla BÃºlgara",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              9,
              9
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-10-10",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11,
              11
            ]
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": [
              13,
              13,
              12
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              2009
            ]
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-10-20",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 140
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 24
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10,
              9
            ]
          },
          "weight": 22.5
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              11,
              11
            ]
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-10-22",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "sentadilla_bulgara",
          "name": "Sentadilla BÃºlgara",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11,
              10
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-10-24",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              2009
            ]
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-10-27",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 150
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 22.5
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11
            ]
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-10-29",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "sentadilla_bulgara",
          "name": "Sentadilla BÃºlgara",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              8
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": [
              13,
              13,
              12
            ]
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-10-31",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              14,
              14,
              12
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              2009
            ]
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-11-03",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 150
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10,
              8
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "t_bar_row",
          "name": "T-Bar Row",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 22.5
        },
        {
          "exercise_id": "press_militar_barra",
          "name": "Press Militar con Barra",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11
            ]
          },
          "weight": 30
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 16,
            "actual": []
          },
          "weight": 17.5
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-11-05",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "sentadilla_bulgara",
          "name": "Sentadilla BÃºlgara",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 0
        },
        {
          "exercise_id": "face_pull",
          "name": "Face Pull",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": [
              13,
              13,
              12
            ]
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-11-07",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_con_cable",
          "name": "Apertura con Cable",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "remo_en_maquina",
          "name": "Remo en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 77
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 55
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              2009
            ]
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-11-10",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 150
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "hammer_row_prono",
          "name": "Hammer Row Prono",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 22.5
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 5
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-11-12",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "hack_squat",
          "name": "Hack Squat",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 5
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 22
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 56
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": [
              13,
              13,
              12
            ]
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-11-14",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_maquina",
          "name": "Apertura en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              9,
              8
            ]
          },
          "weight": 63
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": [
              15,
              15,
              13
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              2009
            ]
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-11-17",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 150
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              10
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "hammer_row_prono",
          "name": "Hammer Row Prono",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 22.5
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 7.5
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 77
        }
      ],
      "completed": true
    },
    {
      "date": "2025-11-19",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "hack_squat",
          "name": "Hack Squat",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 5
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              9
            ]
          },
          "weight": 59
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": [
              13,
              13,
              12
            ]
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-11-21",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_maquina",
          "name": "Apertura en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10,
              9
            ]
          },
          "weight": 63
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": [
              15,
              15,
              13
            ]
          },
          "weight": 14
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 57.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              2009
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              2009
            ]
          },
          "weight": 4.5
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        }
      ],
      "completed": true
    },
    {
      "date": "2025-11-24",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 160
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              11,
              11,
              11
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 7.5
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-11-26",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "sentadilla_bulgara",
          "name": "Sentadilla BÃºlgara",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10,
              9
            ]
          },
          "weight": 24
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "reverse_delt_maquina",
          "name": "Reverse Delt en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 43
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "elevaciones_gemelos",
          "name": "Elevaciones de Gemelos",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": [
              13,
              13,
              12
            ]
          },
          "weight": 165
        }
      ],
      "completed": true
    },
    {
      "date": "2025-11-28",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 84
        },
        {
          "exercise_id": "apertura_maquina",
          "name": "Apertura en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10,
              9
            ]
          },
          "weight": 63
        },
        {
          "exercise_id": "cable_row",
          "name": "Cable Row",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 64
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              2010
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              2010
            ]
          },
          "weight": 4.5
        }
      ],
      "completed": true
    },
    {
      "date": "2025-12-01",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 160
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              11,
              11,
              11
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11,
              9
            ]
          },
          "weight": 59
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 7.5
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-12-03",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "reverse_delt_maquina",
          "name": "Reverse Delt en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 43
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 45
        }
      ],
      "completed": true
    },
    {
      "date": "2025-12-05",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              11,
              9
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 16
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              2010
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              2010
            ]
          },
          "weight": 4.5
        }
      ],
      "completed": true
    },
    {
      "date": "2025-12-08",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 160
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              11,
              11,
              11
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 59
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              11,
              11,
              10
            ]
          },
          "weight": 7.5
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-12-10",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 77
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 24
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "reverse_delt_maquina",
          "name": "Reverse Delt en MÃ¡quina",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 43
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 45
        }
      ],
      "completed": true
    },
    {
      "date": "2025-12-12",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 27.5
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 25
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              2011
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              2010
            ]
          },
          "weight": 4.5
        }
      ],
      "completed": true
    },
    {
      "date": "2025-12-15",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 160
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11,
              11
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 7.5
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-12-17",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 57
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10,
              10
            ]
          },
          "weight": 24
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 64
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 45
        }
      ],
      "completed": true
    },
    {
      "date": "2025-12-19",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 27
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 16
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_mancuernas",
          "name": "Encogimiento de Trapecios con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 4.5
        }
      ],
      "completed": true
    },
    {
      "date": "2025-12-29",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 160
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11,
              11
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 7.5
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2025-12-31",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 57
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 64
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 45
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-02",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 27
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 16
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_mancuernas",
          "name": "Encogimiento de Trapecios con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 4.5
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-05",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 170
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11,
              11
            ]
          },
          "weight": 25
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 7.5
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-07",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 57
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10,
              8
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10
            ]
          },
          "weight": 64
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 23
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-09",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 27.5
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 50
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11
            ]
          },
          "weight": 16
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_mancuernas",
          "name": "Encogimiento de Trapecios con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 34
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 4.5
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-12",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 170
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 27.5
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 7.5
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 84
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-14",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 97
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 57
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10,
              9
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 64
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              8
            ]
          },
          "weight": 23
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-16",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 97
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_mancuernas",
          "name": "Encogimiento de Trapecios con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 36
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 4.5
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-19",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 170
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              11
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 27.5
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 7.5
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-21",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 97
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              10,
              9
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              19
            ]
          },
          "weight": 64
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              11
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 15.25
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 23
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-23",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 97
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 16
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_mancuernas",
          "name": "Encogimiento de Trapecios con Mancuernas",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 40
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 15,
            "actual": []
          },
          "weight": 4.5
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-26",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 170
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              8,
              8
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 27.5
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 7.5
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-28",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 104
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 57
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 64
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 23
        }
      ],
      "completed": true
    },
    {
      "date": "2026-01-30",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 97
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              10,
              9,
              10
            ]
          },
          "weight": 30
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 54
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              14,
              14,
              12
            ]
          },
          "weight": 16
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_barra",
          "name": "Encogimiento de Trapecios con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 6.8
        }
      ],
      "completed": true
    },
    {
      "date": "2026-02-02",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 180
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 27.5
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 8.75
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2026-02-04",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 104
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 63
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              9,
              9
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 68
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 13,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 23
        }
      ],
      "completed": true
    },
    {
      "date": "2026-02-06",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              9,
              8
            ]
          },
          "weight": 97
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              9,
              9
            ]
          },
          "weight": 30
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": [
              14,
              14,
              12
            ]
          },
          "weight": 16
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_barra",
          "name": "Encogimiento de Trapecios con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 6.8
        }
      ],
      "completed": true
    },
    {
      "date": "2026-02-09",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 180
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 27.5
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 8.75
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2026-02-11",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 104
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 63
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 68
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 23
        }
      ],
      "completed": true
    },
    {
      "date": "2026-02-13",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 97
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_barra",
          "name": "Encogimiento de Trapecios con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 6.8
        }
      ],
      "completed": true
    },
    {
      "date": "2026-02-16",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 180
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              9,
              9
            ]
          },
          "weight": 27.5
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2026-02-18",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 104
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 63
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 68
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 23
        }
      ],
      "completed": true
    },
    {
      "date": "2026-02-20",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 97
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 18
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_barra",
          "name": "Encogimiento de Trapecios con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 6.8
        }
      ],
      "completed": true
    },
    {
      "date": "2026-02-23",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 180
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 28
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              9,
              9
            ]
          },
          "weight": 27.5
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2026-02-25",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 104
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 63
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 68
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 23
        }
      ],
      "completed": true
    },
    {
      "date": "2026-02-27",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 97
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 30
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              7
            ]
          },
          "weight": 18
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_barra",
          "name": "Encogimiento de Trapecios con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 6.8
        }
      ],
      "completed": true
    },
    {
      "date": "2026-03-02",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 180
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              9
            ]
          },
          "weight": 28
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              9,
              9
            ]
          },
          "weight": 27.5
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2026-03-04",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 104
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 63
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 68
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 23
        }
      ],
      "completed": true
    },
    {
      "date": "2026-03-06",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 97
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 30
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              7
            ]
          },
          "weight": 18
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_barra",
          "name": "Encogimiento de Trapecios con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 6.8
        }
      ],
      "completed": true
    },
    {
      "date": "2026-03-09",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 180
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              9
            ]
          },
          "weight": 28
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              9,
              9
            ]
          },
          "weight": 27.5
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2026-03-11",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 104
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 63
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 68
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 23
        }
      ],
      "completed": true
    },
    {
      "date": "2026-03-13",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 97
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 30
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              7
            ]
          },
          "weight": 18
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_barra",
          "name": "Encogimiento de Trapecios con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 6.8
        }
      ],
      "completed": true
    },
    {
      "date": "2026-03-16",
      "type": "LUNES",
      "logs": [
        {
          "exercise_id": "prensa_de_piernas",
          "name": "Prensa de Piernas",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 180
        },
        {
          "exercise_id": "press_banca_mancuernas",
          "name": "Press de Banca con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              9
            ]
          },
          "weight": 28
        },
        {
          "exercise_id": "hammer_row_neutral",
          "name": "Hammer Row Neutral",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 27.5
        },
        {
          "exercise_id": "landmine_press",
          "name": "Landmine Press",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 10
        },
        {
          "exercise_id": "curl_femoral_sentado",
          "name": "Curl Femoral Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 90
        },
        {
          "exercise_id": "curl_de_biceps_mancuerna",
          "name": "Curl de BÃ­ceps con Mancuerna",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 12
        },
        {
          "exercise_id": "crunches",
          "name": "Crunches",
          "series": 3,
          "reps": {
            "expected": 18,
            "actual": []
          },
          "weight": 0
        }
      ],
      "completed": true
    },
    {
      "date": "2026-03-18",
      "type": "MIERCOLES",
      "logs": [
        {
          "exercise_id": "abductores",
          "name": "Abductores",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 104
        },
        {
          "exercise_id": "adductores",
          "name": "Adductores",
          "series": 3,
          "reps": {
            "expected": 8,
            "actual": []
          },
          "weight": 63
        },
        {
          "exercise_id": "press_inclinado_mancuernas",
          "name": "Press Inclinado con Mancuernas",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": [
              11,
              11,
              10,
              10
            ]
          },
          "weight": 26
        },
        {
          "exercise_id": "jalon_al_pecho",
          "name": "JalÃ³n al Pecho",
          "series": 4,
          "reps": {
            "expected": 9,
            "actual": []
          },
          "weight": 68
        },
        {
          "exercise_id": "reverse_delt_fly",
          "name": "Reverse Delt Fly",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": [
              15
            ]
          },
          "weight": 9.1
        },
        {
          "exercise_id": "extensiones_de_espalda",
          "name": "Extensiones de Espalda",
          "series": 3,
          "reps": {
            "expected": 14,
            "actual": []
          },
          "weight": 32
        },
        {
          "exercise_id": "elevaciones_laterales_con_cable",
          "name": "Elevaciones Laterales con Cable",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 14
        },
        {
          "exercise_id": "triceps_polea",
          "name": "TrÃ­ceps en Polea",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              10
            ]
          },
          "weight": 23
        }
      ],
      "completed": true
    },
    {
      "date": "2026-03-20",
      "type": "VIERNES",
      "logs": [
        {
          "exercise_id": "extension_de_cuadriceps",
          "name": "ExtensiÃ³n de CuÃ¡driceps",
          "series": 4,
          "reps": {
            "expected": 11,
            "actual": []
          },
          "weight": 97
        },
        {
          "exercise_id": "press_inclinado_maquina",
          "name": "Press Inclinado en MÃ¡quina",
          "series": 4,
          "reps": {
            "expected": 12,
            "actual": [
              12,
              12,
              12,
              10
            ]
          },
          "weight": 30
        },
        {
          "exercise_id": "pullover_polea",
          "name": "Pullover en Polea",
          "series": 4,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 59
        },
        {
          "exercise_id": "press_de_hombros_mancuernas_sentado",
          "name": "Press de Hombros con Mancuernas Sentado",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": [
              10,
              10,
              7
            ]
          },
          "weight": 18
        },
        {
          "exercise_id": "hip_thrust",
          "name": "Hip Thrust",
          "series": 3,
          "reps": {
            "expected": 12,
            "actual": []
          },
          "weight": 57.5
        },
        {
          "exercise_id": "encogimiento_trapecio_barra",
          "name": "Encogimiento de Trapecios con Barra",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 100
        },
        {
          "exercise_id": "overhead_extension_triceps",
          "name": "Overhead Extension TrÃ­ceps",
          "series": 3,
          "reps": {
            "expected": 10,
            "actual": []
          },
          "weight": 6.8
        }
      ],
      "completed": true
    }
  ]
};
